...
const interpolPos = (coordsSource, coordsDest) => {
	...
	let dalka = Math.sqrt(
		horizontalDis * horizontalDis + verticalDis * verticalDis,
	);
	let newHoriz = horizontalDis / dalka;
	let newVertic = verticalDis / dalka;
	return [
		coordsDest[0] + newHoriz * magicShift,
		coordsDest[1] + magicShift * newVertic,
	];
};
...
let main = async () => {
	...
	const light_fetch_loop = async () => {
		...
		setTimeout(light_fetch_loop, 250);
	};
	light_fetch_loop();
};
