...
func main() {
    ...
    app.Post("/save_geojson", func(c *fiber.Ctx) error {
        ...
        err = dao.StoreGeoPaths(DupePaths(resp_paths...)...)
        if err != nil {
            fmt.Printf("fuck")
            return err
        }
...
func DupePaths(lines ...graphconvertor.GeoPath) []graphconvertor.GeoPath {
    out := make([]graphconvertor.GeoPath, 0, len(lines)*2)
    for _, v := range lines {
        out = append(out, v, graphconvertor.GeoPath{Ends: [2]geojson.Coordinate{v.Ends[1], v.Ends[0]}, State: v.State, Size: v.Size, Cars: v.Cars})
    }
    return out
}
